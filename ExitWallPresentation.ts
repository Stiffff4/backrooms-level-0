...
export class ExitWallPresentation {
  ...
  public constructor(
    ...
    this.wallMesh = MeshBuilder.CreateBox(
      `${this.root.name}.wall`,
      {
        width: this.placementValue.width + 0.08,
        height: this.placementValue.height + 0.1,
  ...
  private createGlitchStrip(scene: Scene, rng: SeededRandom, index: number): ExitGlitchStrip {
    ...
    const height = 0.026 + rng.next() * 0.042;
  ...
  private applySample(sample: ExitWallVisualSample): void {
    const diffuseScale = 0.12 + sample.lightResponse * 0.08;
    ...
    this.wallMaterial.emissiveColor.r += sample.emissiveStrength * 0.34;
    this.wallMaterial.emissiveColor.g += sample.emissiveStrength * 0.38;
    this.wallMaterial.emissiveColor.b += sample.emissiveStrength * 0.2;

    this.glitchMaterial.diffuseColor.copyFrom(this.baseDiffuse).scaleInPlace(diffuseScale * 0.42);
    this.glitchMaterial.emissiveColor.copyFrom(this.baseEmissive);
    this.glitchMaterial.emissiveColor.r += sample.emissiveStrength * 1.28;
    this.glitchMaterial.emissiveColor.g += sample.emissiveStrength * 1.34;
    this.glitchMaterial.emissiveColor.b += sample.emissiveStrength * 0.82;
