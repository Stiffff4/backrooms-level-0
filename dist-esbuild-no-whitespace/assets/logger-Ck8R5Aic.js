var Logger = class Logger2 {
  static _CheckLimit(message, limit) {
    ...
    return entry ? entry.current++ : (entry = {
      limit,
      current: 1
    }, Logger2._LogLimitOutputs[message] = entry), entry.current <= entry.limit;
  }
  static _GenerateLimitMessage(message, level = 1) {
    ...
    entry.current === entry.limit && Logger2[type.name](Logger2.MessageLimitReached.replace(/%LIMIT%/g, "" + entry.limit).replace(/%TYPE%/g, type.name ?? ""));
  }
  static _AddLogEntry(entry) {
    Logger2._LogCache = entry + Logger2._LogCache, Logger2.OnNewCacheEntry && Logger2.OnNewCacheEntry(entry);
  }
  static _FormatMessage(message) {
    const padStr = (i) => i < 10 ? "0" + i : "" + i, date = /* @__PURE__ */ new Date();
  ...
  static _LogEnabled(level = 1, message, limit) {
    ...
    const formattedMessage = Logger2._FormatMessage(msg), type = this._Levels[level], optionals = Array.isArray(message) ? message.slice(1) : [];
    type.logFunc && type.logFunc("BJS - " + formattedMessage, ...optionals);
    const entry = `<div style='color:${type.color}'>${formattedMessage}</div><br>`;
    Logger2._AddLogEntry(entry), Logger2._GenerateLimitMessage(msg, level);
  }
  static get LogCache() {
    return Logger2._LogCache;
  }
  static ClearLogCache() {
    Logger2._LogCache = "", Logger2._LogLimitOutputs = {}, Logger2.errorsCount = 0;
  }
  static set LogLevels(level) {
    Logger2.Log = Logger2._LogDisabled, Logger2.Warn = Logger2._LogDisabled, Logger2.Error = Logger2._LogDisabled;
    const levels = [
