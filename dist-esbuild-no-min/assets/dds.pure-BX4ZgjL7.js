...
import { t as Clamp } from "./math.scalar.functions-BxWMGrCV.js";
import { t as CubeMapToSphericalPolynomialTools } from "./cubemapToSphericalPolynomial-C06arCBE.js";
...
var DDSTools = class DDSTools2 {
  ...
  static _GetHalfFloatAsFloatRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
    ...
    for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
      ...
      if (DDSTools2.StoreLODInAlphaChannel) destArray[index + 3] = lod;
  ...
  static _GetFloatAsHalfFloatRGBAArrayBuffer(width, height, dataOffset, dataLength, arrayBuffer, lod) {
    ...
    for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
      ...
      if (DDSTools2.StoreLODInAlphaChannel) destArray[index + 3] = ToHalfFloat(lod);
  ...
  static UploadDDSLevels(engine, texture, data, info, loadMipmaps, faces, lodIndex = -1, currentFace, destTypeMustBeFilterable = true) {
    ...
    const rOffset = DDSTools2._ExtractLongWordOrder(header[off_RMask]);
    const gOffset = DDSTools2._ExtractLongWordOrder(header[off_GMask]);
    const bOffset = DDSTools2._ExtractLongWordOrder(header[off_BMask]);
    const aOffset = DDSTools2._ExtractLongWordOrder(header[off_AMask]);
          ...
          if (!info.isCompressed && info.isFourCC) {
            ...
            if (engine._badOS || engine._badDesktopOS || !caps.textureHalfFloat && !caps.textureFloat) {
              if (bpp === 128) {
                ...
                if (sphericalPolynomialFaces && i == 0) sphericalPolynomialFaces.push(DDSTools2._GetFloatRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i));
              } else if (bpp === 64) {
                floatArray = DDSTools2._GetHalfFloatAsUIntRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i);
                if (sphericalPolynomialFaces && i == 0) sphericalPolynomialFaces.push(DDSTools2._GetHalfFloatAsFloatRGBAArrayBuffer(width, height, data.byteOffset + dataOffset, dataLength, data.buffer, i));
              }
              texture.type = 0;
            } else {
              ...
              switch (bpp) {
                case 128:
                  switch (destType) {
                    case 1:
                      dataGetter = DDSTools2._GetFloatRGBAArrayBuffer;
                      dataGetterPolynomial = null;
                      break;
                    case 2:
                      dataGetter = DDSTools2._GetFloatAsHalfFloatRGBAArrayBuffer;
                      dataGetterPolynomial = DDSTools2._GetFloatRGBAArrayBuffer;
                      break;
                    case 0:
                      dataGetter = DDSTools2._GetFloatAsUIntRGBAArrayBuffer;
                      dataGetterPolynomial = DDSTools2._GetFloatRGBAArrayBuffer;
                      break;
                  }
                  break;
                default:
                  switch (destType) {
                    case 1:
                      dataGetter = DDSTools2._GetHalfFloatAsFloatRGBAArrayBuffer;
                      dataGetterPolynomial = null;
                      break;
                    case 2:
                      dataGetter = DDSTools2._GetHalfFloatRGBAArrayBuffer;
                      dataGetterPolynomial = DDSTools2._GetHalfFloatAsFloatRGBAArrayBuffer;
                      break;
                    case 0:
                      dataGetter = DDSTools2._GetHalfFloatAsUIntRGBAArrayBuffer;
                      dataGetterPolynomial = DDSTools2._GetHalfFloatAsFloatRGBAArrayBuffer;
                      break;
                  }
                  break;
